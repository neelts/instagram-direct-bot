...
GetAvatar.main = function() {
	GetAvatar.bot = new instagram_bot_DirectBot("get.avatar",[GetAvatarResponder]);
};
var instagram_bot_DirectBotComponent = function(bot) {
	this.bot = bot;
};
instagram_bot_DirectBotComponent.__name__ = true;
var instagram_bot_DirectResponder = function(bot) {
	this.queue = [];
	instagram_bot_DirectBotComponent.call(this,bot);
	this.inbox = new instagram_Inbox(bot.session);
	this.threadItems = new instagram_thread_ThreadItems(bot.session);
	this.get();
};
instagram_bot_DirectResponder.__name__ = true;
instagram_bot_DirectResponder.__super__ = instagram_bot_DirectBotComponent;
instagram_bot_DirectResponder.prototype = $extend(instagram_bot_DirectBotComponent.prototype,{
	get: function() {
		var _gthis = this;
...
var GetAvatarResponder = function(bot) {
	instagram_bot_DirectResponder.call(this,bot);
};
GetAvatarResponder.__name__ = true;
GetAvatarResponder.__super__ = instagram_bot_DirectResponder;
GetAvatarResponder.prototype = $extend(instagram_bot_DirectResponder.prototype,{
	check: function(thread,message) {
...
google_Analytics.__name__ = true;
google_Analytics.event = function(category,action,user) {
	if(user == null) {
		user = "0";
	}
	...
	process.stdout.write(v);
	process.stdout.write("\n");
	if(google_Analytics.request != null) {
		google_Analytics.request.setParameter("cid",user);
		google_Analytics.request.setParameter("ec",category);
		google_Analytics.request.setParameter("ea",action);
		google_Analytics.request.request(true);
	}
};
...
var instagram_bot_DirectApprover = function(bot) {
	instagram_bot_DirectBotComponent.call(this,bot);
	this.inboxPending = new instagram_InboxPending(bot.session);
	this.get();
};
instagram_bot_DirectApprover.__name__ = true;
instagram_bot_DirectApprover.__super__ = instagram_bot_DirectBotComponent;
instagram_bot_DirectApprover.prototype = $extend(instagram_bot_DirectBotComponent.prototype,{
	get: function() {
	...
	,check: function(threads) {
		...
		while(_g < threads.length) {
			...
			google_Analytics.event("pending","approve",haxe_crypto_Sha256.encode(thread.accounts[0].id));
		}
		haxe_Timer.delay($bind(this,this.get),(2 + Math.random() * 3) * 1000 | 0);
	}
	,error: function(e) {
...
var instagram_bot_DirectBot = function(user,components) {
	this.user = user;
	this.components = components;
	this.components.unshift(instagram_bot_DirectApprover);
...
instagram_bot_DirectBot.__name__ = true;
instagram_bot_DirectBot.prototype = {
	login: function() {
		var _gthis = this;
		google_Analytics.event("app","login");
	...
	,init: function(userId) {
		...
		google_Analytics.event("app","init");
		new haxe_Timer(60000).run = function() {
			google_Analytics.event("app","alive");
			return;
		};
	}
};
...
google_Analytics.ga = "ga.id";
google_Analytics.request = sys_FileSystem.exists("ga.id") ? (function($this) {
	var $r;
...
instagram_bot_DirectBot.COOKIES = "cookies";
instagram_bot_DirectBot.alive = 60000;
GetAvatar.main();
})();
